...
def truesort(name):
    ...
    (well_num, electrode_num) : int, int
        These are the numbers of the wells and the numbers of the elctrode, sorted.   
...
def make_hm(Vars, Classes, Colour_classes):    
    ...
    ani : matplotlib.animation.FuncAnimation
        The animation that shows the heatmap. 
    ...
    # Setup plot with 6x8 grid of subplots
    fig, axs = plt.subplots(Vars["Rows"], Vars["Cols"], figsize=(12, 9), facecolor='black')
    fig.subplots_adjust(wspace=0.009, hspace=0.009)

    # factor = 1.2
    # fig.set_size_inches(cols*factor, rows*factor)  # Set the "container" size for your plot
    axs = axs.ravel()  # Flatten to 1D array

    heatmaps = []
    i = 0
    for ax in axs:
        # Remove ticks so it won't interfere with the plots
        ax.set_xticks([])
        ax.set_yticks([])

        # Sets lims so that the plots are certain to be within the limits
        ax.set_xlim(-0.5, 3.5)
        ax.set_ylim(3.5, -0.5)
    ...
    # Makes the colour bar on the side, labeling it with the params.
    pos = ax.get_position()
    cbar = plt.colorbar(plt.cm.ScalarMappable(cmap=Vars["cmap"], norm=plt.Normalize(vmin, vmax)), ax=axs, cax=fig.add_axes([pos.x1 + 0.02, pos.y0, 0.02, (pos.height * Vars["Rows"])]))
...
def create_placeholder_figure(Vars):
    ...
    pos = ax.get_position()
    cbar = plt.colorbar(plt.cm.ScalarMappable(cmap=Vars["cmap"], norm=plt.Normalize(0, 1)), 
                        ax=axs, 
